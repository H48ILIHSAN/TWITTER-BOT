...
INSTA_USERNAME = 'jkt48.flora'
INSTA_ID       = '14811899509'
...
def getTwitAPI():
    with open(absPath('config/twitterAPIFlora.txt')) as twitData:
...
def ReadLastTweet():
    if not os.path.exists(absPath('tempFlora.txt')):
        return 0
    with open(absPath('tempFlora.txt')) as file:
        read = file.read()
        timestamp = str(read)
    return timestamp

def deleteStory():
    shutil.rmtree(absPath('assetsFlora/'))
    os.makedirs(absPath('assetsFlora/'))
    with open('assetsFlora/story','w') as file:
        file.write('')
        ...
        if storyData['items'] is not None:
            for i in storyData['items']:
                ...
                if lastStory >= str(storyID):
                    ...
                    continue
                storyGET = absPath('assetsFlora/'+storyID+'.mp4')
                if i["media_type"] == 1:
                    time.sleep(30)
                    url = i['image_versions2']['candidates'][0]['url']
                    urllib.request.urlretrieve(url, storyGET)
                    twitMedia(storyGET)
                    with open('tempFlora.txt','w') as file:
                        write = str(storyID)+'\n'
                        file.write(write)
                elif i["media_type"] == 2:
                    time.sleep(30)
